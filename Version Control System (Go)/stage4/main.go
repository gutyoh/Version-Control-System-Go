...
const (
	...
	SwitchedToCommitMessage = "Switched to commit %s.\n"
)
...
func (*VersionControlSystem) Checkout(args []string) {
	if len(args) < 2 {
	...
	if err != nil {
		log.Printf("cannot read commit directory: %v\n", err)
		return
	}

	for _, file := range files {
		fileInCommitPath := filepath.Join(commitPath, file.Name())
		content, err := os.ReadFile(fileInCommitPath)
		if err != nil {
			log.Printf("cannot read file in commit: %v\n", err)
			return
		}

		err = os.WriteFile(file.Name(), content, os.ModePerm)
		if err != nil {
			log.Printf("cannot restore file: %v\n", err)
			return
		}
	}

	fmt.Printf(SwitchedToCommitMessage, commitID)
}
